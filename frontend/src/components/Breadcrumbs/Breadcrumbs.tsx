...
export const BreadcrumbsComponent: FC<IBreadcrumbsComponentProps> = (props) => {
  ...
  useEffect(() => {
    if (
      ...
      instrument === instrumentTypes.reconstruction
    ) {
      ...
      <nav className="menu">
        <ul className="menu__list">
          ...
          >
            ...
            <span className="separator no_select">/</span>
            <ul
              className="sub-menu__list left-menu"
              style={isTimeMenuOpen ? { display: "block" } : null}
            >
                    ...
                    {time === timeTypes.earthHistory ? (
                      <li className="sub-menu__link no_select" key={index}>
                        ...
                        <ul
                          ...
                          style={
                            isTimeSubMenuOpen ? { display: "block" } : null
                          }
                        >
                      ...
                      <li
                        className={clsx(
                          "sub-menu__link",
                          "no_select",
                          isEarthOrReliefPicked ? "disabled" : ""
                        )}
                        key={index}
                      >
                        <Link to={`/${pageRedirect(time, instrument)}`}>
          ...
          >
            ...
            <ul
              className={clsx("sub-menu__list")}
              style={isInstrumentMenuOpen ? { display: "block" } : null}
            >
              {currentInstruments.map((instrument, index) => {
                return (
                  ...
                  >
                    <Link to={`/${pageRedirect(time, instrument)}`}>
                      <span
                        onClick={() => changeBreadcrumbsInstrument(instrument)}
                      >
                        {instrument}
                      </span>
                    </Link>
